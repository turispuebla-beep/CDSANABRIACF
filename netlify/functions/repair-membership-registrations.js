...
const { repairMembershipsFromPaymentOrders } = require('./lib/firestore-admin');
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const result = await repairMembershipsFromPaymentOrders(orderIds);
    return jsonResponse(200, result, origin);
  } catch (err) {
    console.error('repair-membership-registrations:', err);
